...
%  compress-and-forward 05 let s0 of the orignal and the compressed are
%  same for approximation
SCALE = 1000 ;
T_GCL = 1 ;
eta = 2/3 ;
L_TT = T_GCL - eta ;
...
%%
r = 5/8 ;  %when eta = 2/3 ; 3/4 > (eta = 2 / 3 ) > 5/8 > ( 2-eta^-1 = 1/2 ) > 1/3
%%
R = I_CBS / r ;
x0 = mod( o , T_GCL ) * T_GCL ;
% s0 = ( T_GCL - mod( o , T_GCL ) ) * R ;
...
if ( r >= eta )
    s = eta * R ;
   c2_inv =   rtccurve( [ [0*SCALE  o*SCALE 1/1/I_CBS ]  ] , ...
    [ [0*SCALE 0*SCALE 1/R] ] , ...
     s0* SCALE ,      (t0 + L_TT)* SCALE ,      round ( s * SCALE ),   T_GCL * SCALE ) ;
else
...
ylabel('time');
title('Note: approximation with the same s_0' );
