...
c = categorical({'30','60','90','120'});


 TT_delay = [55 32 32;59 38 32;62 45 33;72 52 42] ;
 subplot(211)
...
ylim([0 80])                % X-Axis Limit 
yticks(0:10:80);         % Increment Limit in the Y-Axis
...
xlab=xlabel('Number of Message in the network'); % Text Title of the X-Label
...
ylab=ylabel('Monetary Cost(ms)'); % Text Title of the Y-Label
...
h=legend('\boldmath $NLR $ ','\boldmath $ Proposed Algorithm $ ','\boldmath $ SS $ '); % Legend name inside the box
...
%t = title('\boldmath $ NLR $ \boldmath $ to $ \boldmath $ End $ \boldmath $ delay $ \boldmath $ of $ \boldmath $ TT $ \boldmath $ for $ \boldmath $ TAS+CBS $')
